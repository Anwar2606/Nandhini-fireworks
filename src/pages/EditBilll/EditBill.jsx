...
//   const formattedDate = formatDate(bill.createdAt);
  ...
  doc.rect(borderMargin, borderMargin, pageWidth - 2 * borderMargin, pageHeight - 2 * borderMargin);
      ...
      doc.text('NANDHINI FIREWORKS', 44, 21);
      doc.setTextColor(0, 0, 0);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.text('559, Kankarseval, Sivakasi-626 123', 44, 28);
      doc.setFontSize(9);
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.text(`Invoice Number: NF-${bill.invoiceNumber || 'N/A'}-24`, 138, 43);
      doc.text(`Date: ${formattedDate}`, 138, 36);
      doc.setFont('helvetica', 'bold');
      doc.text('GSTIN: 33CQSPM0068G1ZP', 138, 49);
    ...
    const customerDetails = {
      ...
      Object.entries(shippedDetails).forEach(([label, value]) => {
        ...
        doc.text(':', initialX + 20, initialY);
        doc.text(value, initialX + 24, initialY);
        initialY += lineHeight;
      });
    ...
    //   ]);
    const tableBody = bill.productsDetails.map(item => {
        const saleprice = typeof item.saleprice === 'string' || typeof item.saleprice === 'number' ? item.saleprice : 0; // Ensure saleprice is a valid number
        const quantity = typeof item.quantity === 'string' || typeof item.saleprice === 'number' ? item.quantity : 0; // Ensure quantity is a valid number
      
        return [
          item.name || 'N/A',
          '36041000', // You can replace this with the actual HSN code if needed
          quantity.toString(), // Convert quantity to string for display
          `Rs.${saleprice}`.toString(), // Ensure saleprice is a valid number and format it
          `Rs.${(quantity * saleprice)}`.toString() // Calculate the total amount
        ];
      });
